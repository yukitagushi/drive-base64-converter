...
async function safeFetch(url, options = {}) {
  ...
  if (!response.ok) {
    const message =
      typeof body === 'string'
        ? body || response.statusText || 'リクエストに失敗しました'
        : body?.error || response.statusText || 'リクエストに失敗しました';
...
function toggleScreenVisibility(element, show) {
  if (!element) {
    return;
  }
  const active = document.activeElement;
  if (!show && active && element.contains(active) && typeof active.blur === 'function') {
    active.blur();
  }
  element.hidden = !show;
  if (show) {
...
function updateAuthUi() {
  ...
  toggleScreenVisibility(landingScreen, showLanding);
  toggleScreenVisibility(loginScreen, showLogin);
  if (authTrigger) {
  ...
  try {
    const data = await safeFetch('/api/chat', {
      ...
      body: JSON.stringify({
        ...
        stores: storeCache.map((store) => store.name).filter(Boolean),
        session: {
...
async function loadStores(options = {}) {
  ...
  try {
    const query = sessionState.officeId ? `?officeId=${encodeURIComponent(sessionState.officeId)}` : '';
    const data = await safeFetch(`/api/file-stores${query}`);
    if (data?.error) {
...
async function onUploadFile(event) {
  ...
  try {
    ...
    if (data.file?.analysis?.summary) {
      setStatus(`Gemini 画像解析: ${data.file.analysis.summary}`);
    } else {
      setStatus('ファイルをアップロードしました');
    }
  } catch (error) {
    console.error(error);
...
function updateMessage(messageRefs, text, context) {
  ...
  if (context && Array.isArray(context) && context.length > 0) {
    ...
    context.forEach((item, index) => {
      ...
      const parts = [`ソース${index + 1}: ${item.title || '関連ドキュメント'}`];
