...
class SupabaseService {
  ...
  async listOfficeFileStores(officeId) {
    if (!officeId) {
      return [];
    }
    ...
    const params = new URLSearchParams();
    params.set('select', 'id,office_id,gemini_store_name,display_name,description,created_at');
    params.set('office_id', `eq.${officeId}`);
    ...
    return rows.map((row) => ({
      ...
      displayName: row.display_name || row.gemini_store_name,
  ...
  async decorateStoresForOffice(stores, officeId) {
    ...
    return stores
      .filter((store) => {
        ...
        if (!record) {
          // 古いストアや Supabase で未登録のストアはそのまま表示する。
          return true;
        }
        // 別オフィスに紐づくストアだけは一覧から除外する。
        return record.office_id === officeId;
      })
      .map((store) => {
        const record = recordMap.get(store.name);
        if (!record || record.office_id !== officeId) {
          return store;
        }
        return {
          ...store,
