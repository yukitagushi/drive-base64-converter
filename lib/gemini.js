...
class GeminiFileSearchService {
  ...
  async uploadFile({ storeName, fileName, mimeType, data, description }) {
    ...
    if (isImage) {
      analysis = await this.analyzeImage({ data, mimeType });
      const analysisText = this.#formatImageAnalysis({
        originalFileName: fileName,
        mimeType,
        analysis,
      });
      uploadFileName = deriveAnalysisFileName(fileName);
      uploadBlob = new Blob([analysisText], { type: 'text/plain; charset=utf-8' });
      uploadDescription = uploadDescription || analysis.summary || 'Gemini による画像説明';
    } else {
    ...
    if (uploadDescription) {
      metadata.description = uploadDescription;
    }
  ...
  #formatImageAnalysis({ originalFileName, mimeType, analysis }) {
    const lines = [];
    lines.push(`# 画像の説明 (${new Date().toISOString()})`);
    if (originalFileName) {
...
function deriveAnalysisFileName(original = '') {
  const trimmed = String(original || '').trim();
  if (!trimmed) {
    return 'image-analysis.txt';
  }
