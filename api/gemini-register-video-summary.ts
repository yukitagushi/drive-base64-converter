...
const API_NAME = '/api/gemini-register-video-summary';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (!isVideoMime(fileRow.mime_type)) {
      respond(res, 400, { error: '動画ファイルのみ処理できます。' });
      return;
    }

    const result = await ensureVideoSummaryForFile({
      admin,
      supabase,
      fileRow,
      storeRow,
    ...
    respond(res, status, {
      error: 'register_video_summary_failed',
      message: error?.message || '動画文字起こしテキストの登録に失敗しました。',
